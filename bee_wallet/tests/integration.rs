...
//
// Native-only: pulls the tvm/kit graph and its native dev-deps live behind
// `cfg(not(target_arch = "wasm32"))`. Compile to an empty crate on wasm32 so
// `wasm-pack test` (which forces `--tests`) doesn't try to build them.
