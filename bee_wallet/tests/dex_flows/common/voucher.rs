...
//!
//! Production code emits the voucher through a multifactor wallet and
//! drives `prove_voucher_for_event` directly (see
//! `flows::test_production_flow_voucher_deploy_pn_and_stake`). This
//! helper only exists so the rest of the integration tests don't have to
//! stand up a full multifactor wallet for every voucher they need.
...
/// Production-grade voucher cycle: multifactor wallet emits the voucher,
/// halo2 proves against the wallet-emitted event. Picks a fresh `sk_u`
/// per call so each voucher binds to its own commitment.
#[allow(clippy::too_many_arguments)]
pub async fn mint_voucher_via_multifactor(
    wallet: &bee_wallet::Wallet,
    mf_address: &str,
    mf_keys: &KeyPair,
    ...
    wallet
        .generate_voucher(bee_wallet::ParamsOfGenerateVoucher {
            multifactor_address: mf_address.to_string(),
            token_type,
            amount,
            is_fee,
            sk_u_commit: format!("0x{sk_u_commit_hex}"),
            signer_keys: mf_keys.clone(),
        })
        .await
        .expect("wallet.generate_voucher");

    // Locate OUR voucher event by `skUCommit` match — bulletproof under
    // parallel test runs because each call generates a fresh random
    // commitment (collision-free vs. timestamp-window heuristics).
    ...
    .await
    .expect("wait_for_voucher_event_by_sk_u_commit (multifactor flow)");
