...
fn module_prefix(module: KitModule) -> &'static str {
    match module {
        ...
        // Downstream-owned modules (e.g. relocated DEX wrappers) carry their
        // own stable id as the prefix.
        KitModule::External(name) => name,
        // `KitModule` is `#[non_exhaustive]`; tolerate future variants.
        _ => "unknown",
    }
}
